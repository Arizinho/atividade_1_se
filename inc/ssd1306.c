...
void ssd1306_rect(ssd1306_t *ssd, uint8_t top, uint8_t left, uint8_t width, uint8_t height, bool value, bool mode) {
  ...
  if (mode) {
    uint8_t countador = 0;
    for (uint8_t x = left; x < left + width; ++x) {
      ssd1306_pixel(ssd, x, top, !(countador>>2));
      ssd1306_pixel(ssd, x, top + height - 1, !(countador>>2));
      countador = (countador + 1) % 5;
    }
    countador = 0;
    for (uint8_t y = top; y < top + height; ++y) {
      ssd1306_pixel(ssd, left, y, !(countador>>2));
      ssd1306_pixel(ssd, left + width - 1, y, !(countador>>2));
      countador = (countador + 1) % 5;
    }
  }
}
...
// Função para desenhar uma ponto 8x8
void ssd1306_draw_dot(ssd1306_t *ssd, uint8_t x, uint8_t y)
{
